...
class Attribute extends Yj
{
	...
	public function change_info($id=0,$field='title',$value=''){
			$id  || $this->error('id is empty');
			$value || $this->error('name is empty');
			db('attribute')->where('id',$id)->update([$field=>$value])?$this->success('修改成功'):$this->err('修改失败');
	}
}
