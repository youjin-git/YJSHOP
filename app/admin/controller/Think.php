...
class Think extends Yj{
        public function base_lists($model=0){
				...
				//高级搜索
				if($model['search_list']){
					($search_list = db('attribute')->where('name',$model['search_list'])->where('model_id',$model['id'])->value('extra'))&&$this->assign('search_list',parse_field_attr($search_list));
					isset($get[$model['search_list']])&&$map[$model['search_list']] = $get[$model['search_list']];
				}
                foreach($search_key as &$value){
                		if(empty($value))continue;
                		 ...
                		 list($value['type'],$value['extra']) = db('attribute')->where('name',$value['key'])->where('model_id',$model['id'])->field(['type'=>0,'extra'=>1])->find();
						 if($value['type']=='string'){
                	  	 		if(isset($get[$value['key']])&&!empty($get[$value['key']])){
                	  	 			$map[$value['key']] = ['like','%'.$get[$value['key']].'%'];
                	  	 		}
                	  	 }elseif($value['type']=='datetime'||$value['type']=='date'){
                	  	 		 $a = $b = [];
                	  	 		...
                	  	 		if(isset($get[$value['key']])&&trim($get[$value['key']])!==''){
	  ...
	  public function base_update($model=0){
	  			$model || $this->error('模型名标识必须');
       			$model =  db('model')->getByName($model);
       			$model || $this->error('模型不存在');
      			 ...
      			 foreach($fields as $key=>$attr){
      			 	   if($attr['is_must']){// 必填字段
      			 		...
      			 		if($attr['validate_rule']){
      			 			$validate[] = array($attr['name'],$attr['validate_rule']);
      			 		}
      			 		if(!empty($attr['auto_rule'])){
      			 			switch($attr['auto_type']){
								default:
								
								(($attr['auto_time']==1&&$method=='add')||($attr['auto_time']==2&&$method=='edit')||($attr['auto_time']==3))&&($post[$attr['name']] = $this->auto_time($attr['auto_rule'],isset($post[$attr['name']])?$post[$attr['name']]:''));
								break;
							}
      			 		}else{
      			 			switch($attr['type']){
	  ...
	  public function auto_time($auto_type,$value){
	  		switch($auto_type){
				case 'time':
				$value = time();			
				break;
				case 'md5':
				$value = (strlen(trim($value))<6||strlen($value)==32)?$value:md5($value);
				break;
				default:
