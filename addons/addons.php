...
abstract class Addons
{
    ...
    {
        // 获取当前插件目录
        $this->addons_path = ROOT_PATH .'addons'.DS.$this->getName() . DS;
        // 读取当前插件配置信息
        if (is_file($this->addons_path . 'config.php')) {
            $this->config_file = $this->addons_path . 'config.php';
        }
        $TMPL_PARSE_STRING['__ADDONROOT__'] ='/Addons/'.$this->getName();
    ...
    public function display($content, $vars = [], $replace = [], $config = [])
    {
        ...
        echo $this->view->fetch(DS.$content, $vars, $replace, $config);
    }
